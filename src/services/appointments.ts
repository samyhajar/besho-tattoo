...
// Email functions are now handled server-side via API routes
...
export async function createPublicAppointment(
  ...
  try {
    ...
    // Email sending is now handled server-side via API routes
    console.log("📧 Email sending handled by server-side API route");
...
export async function confirmAppointment(appointmentId: string): Promise<void> {
  ...
  // Email sending is now handled server-side via API routes
  console.log("📧 Confirmation email handled by server-side API route");
}
