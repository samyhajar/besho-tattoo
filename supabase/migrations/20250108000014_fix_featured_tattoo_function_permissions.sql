-- Migration: Fix featured tattoo function permissions
-- Purpose: Add SECURITY DEFINER to functions to bypass RLS when accessing tattoos table
-- Created: 2025-01-08
...
BEGIN
  ...
  FROM public.featured_tattoo ft
  JOIN public.tattoos t ON ft.tattoo_id = t.id
  LIMIT 1;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  DELETE FROM public.featured_tattoo;
  
  -- Insert the new featured tattoo
  INSERT INTO public.featured_tattoo (tattoo_id)
...
RETURNS TABLE (is_featured boolean) 
SECURITY DEFINER
SET search_path = ''
AS $$
BEGIN
  RETURN QUERY
  SELECT EXISTS(
    SELECT 1 FROM public.featured_tattoo WHERE tattoo_id = tattoo_id_param
  ) as is_featured;
END;
$$ LANGUAGE plpgsql;
